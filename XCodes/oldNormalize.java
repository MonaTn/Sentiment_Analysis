...
public class oldNormalize {

	private double[] minValues;
	private double[] maxValues;
	private int arraySize = Constant.getFeatureSize();

	public List<double[]> exteractMinMaxValues(CaseBase caseBaseList) {
		double[] initialValues = caseBaseList.getCases().get(0)
				.getCaseDescription().getFeatures();
		minValues = Arrays.copyOf(initialValues, initialValues.length);
		maxValues = Arrays.copyOf(initialValues, initialValues.length);
		;
		for (Case caseBase : caseBaseList.getCases()) {
			findMinMaxValues(caseBase.getCaseDescription().getFeatures());
		}
	...
	private void findMinMaxValues(double[] features) {
