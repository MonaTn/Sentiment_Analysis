...
public class NegationPreprocessor_3009013 {
...
//		macros.addMacro("@T", "’t|'t|t|’T|'T|T");
	...
	public   List<String> getPatternsList() {
		...
		patternsList.add("VP=head << NP=np : (=np << JJ=adj) : (=adj < @Few !$, @A !$, @VERY)"); // in object
		patternsList.add("VP=head $, NP=np : (=np << JJ=adj) : (=adj < @Few !$, @A !$, @VERY)"); // in subject
		/** Little 1 pattern **/
		...
		patternsList.add("VP=head $, NP=np : (=np < (DT=dt < @NO)) : (=dt [!.. /,/ & !.. /:/])"); // ##1
		...
		patternsList.add("VP=head $, @AD=ad : (=ad < (__ < @NO))");
		patternsList.add("NP=head < __=label : (=label < @NO)"); //?
...
//		patternsList.add("NP=head < NP=np < PP=pp : (=np <+(NN) @Lack $. =pp) : (=pp <+(IN) of|Of|OF)");
	...
	private final   String[] negationCues = {"no","not", "n't", "n’t", "never", "barely", "scarcely", "hardly", "nothing",
		"neither", "nor", "few", "little", "nobody", "none", "nowhere", "without", "lack", "lacked", "lacking", "lacks"};
