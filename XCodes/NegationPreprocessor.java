...
public class NegationPreprocessor {
	...
	public   TregexPatternCompiler getMacros1() {
		TregexPatternCompiler macros = new TregexPatternCompiler();
		macros.addMacro("@VB", "/^VB/");
		macros.addMacro("@AD", "ADVP|ADJP");
		macros.addMacro("@PRN", "NP|ADJP|ADVP|S|PP");
...
//		patternsList.add("NP=head < NP=np < PP=pp : (=np <+(NN) /^(?i:lack)$/ >, =head) : (=pp <+(IN) /^(?i:of)$/ >- =head)");
	...
	public   List<String> getAuxiliaryPattern(){  // deleted in NegationAnalysis 
		List<String> patternsList =  new ArrayList<String>();
		patternsList.add("VP=h < @VB=vb < @RB-NOT < VP : (=vb [< @Aux | < @Be)"); // auxiliary (to be/have/do)
