...
public class NegationPreprocessor_0310013 {
	...
	public   TregexPatternCompiler getMacros() {
		...
		macros.addMacro("@Be", "am|'m|’m|ai|is|are|was|were|'s|’s|s|'re|’re");
		...
		macros.addMacro("@VERY", "RB < VERY|Very|very");
		macros.addMacro("@PUNC", ":|.|,");
		return macros;
	}
	...
	public   List<String> getPatternsList() {
		...
		patternsList.add("VP=head < @VB=vb < VP=vp : (=vb < @Aux .. RB=rb) : (=rb < @NOT=not) : (=not >> =head) : (=vp < @VB)"); // !>> S)"); // VP-aux (have/do) not VP
		...
		/** No 7 patterns**/
		patternsList.add("S=head < NP=np < VP=vp : (=np >, =head <+(DT) @NO) : (=vp >- =head !< VP)");//*
...
//		patternsList.add("PP=head , @NOOT");
		...
		patternsList.add("@AD=head $,, @RB-NOT : (=head >> FRAG=frag) : (=frag !$, WHADVP)");
		...
		/** Lack **/
		patternsList.add("NP=head < JJ=jj : (=jj < /^(?i:lack)$/)");
//		patternsList.add("NP=head < NP=np < PP=pp : (=np <+(NN) @Lack $. =pp) : (=pp <+(IN) of|Of|OF)");
		patternsList.add("NP=head < NP=np < PP=pp : (=np <+(NN) /^(?i:lack)$/ >, =head) : (=pp <+(IN) /^(?i:of)$/ >- =head)");
		patternsList.add("PP=head $, NP=np > NP=parent : (=np <+(NN) /^(?i:lack)$/ > =parent) : (=head <+(IN) of|Of|OF)");
		patternsList.add("VP=head < @VB=vb < NP : (=vb < /^(?i:lack)$/)");
		patternsList.add("NP=head <+(NP) __=label < NP : (=label < /^(?i:lack)$/ >>, =head)");
	...
	public   boolean containNegationCues(List<HasWord> sentence) {
		for (HasWord word : sentence) {
			if ( isNegationCues(word.word().toLowerCase())) {
				return true;
			}
		}
		return false;
	...
	public boolean isNegationCues (String word) {
		return Arrays.asList(negationCues).contains(word.toLowerCase());
	}

	private final   String[] negationCues = {"no","not", "n't", "n’t", "nope", "never", "barely", "scarcely", "hardly", "nothing",
		"neither", "nor", "nobody", "none", "nowhere", "without", "lack", "lacked", "lacking", "lacks"};
}
